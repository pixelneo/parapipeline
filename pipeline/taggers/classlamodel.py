...
class ClasslaTagger(BaseTagger):
    ...
    def _check_model(self, lang):
        ...
        if lang not in self.models:
            ...
            self.models[lang] = classla.Pipeline(lang=lang_code, models_dir=os.path.join(FILE_PATH, 'classla', 'models'))
    ...
    def process(self, text: str, lang:str, in_format=None, out_format=None):
        ...
        for s_raw in text.strip().split('\n'):
            ...
            for s2 in s.sentences:  # sentence segmentation is performed
                for t in s2.tokens:
                    for w in t.words:
                        src = (('word', w.text), ('lemma', w.lemma), ('upos', w.upos), ('xpos', w.xpos), ('feats', w.feats))
                        a = [(k, v) for k,v in src if v is not None ]
                        res.append(dict(a))
                        #res.append({'word': a[0], 'lemma': a[1], 'upos': a[2], 'xpos': a[3], 'feats': a[4]})
            yield res
